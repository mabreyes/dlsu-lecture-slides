...
class MLPVisualizer {
  ...
  calculateLayerPositions(layers) {
    ...
    const layerXPositions = d3.range(layers.length).map(i => 
      padding + (i * (availableWidth / (layers.length - 1)))
    );
  ...
  drawNeurons(layerPositions) {
    ...
    layerPositions.forEach((positions, layerIndex) => {
      // Draw neurons
      const neurons = this.svg.selectAll(`.layer-${layerIndex}`)
        .data(positions)
        .enter()
        .append("circle")
        .attr("class", d => {
      ...
      // Add neuron labels
      this.svg.selectAll(`.neuron-label-${layerIndex}`)
        .data(positions)
        .enter()
        .append("text")
      ...
      nextLayerPositions.forEach((nextNeuron, nextIndex) => {
        ...
        currentLayerPositions.forEach((currentNeuron, currentIndex) => {
          ...
          this.svg.append("line")
            ...
            .style("stroke-width", Math.max(0.5, Math.min(lineWidth, 4)))
  ...
  addLayerLabels(layerPositions) {
    const layerNames = [
      "Input Layer", 
      ...Array(layerPositions.length - 2).fill().map((_, i) => `Hidden Layer ${i + 1}`),
      "Output Layer"
    ];
