...
class MLPVisualizerApp {
  ...
  /**
   * Set moons classification data
   */
  setMoonsData() {
    // Get moons data
    this.trainingData = DataGenerator.generateMoonsData(200);
  ...
  /**
   * Set Gaussian clusters classification data
   */
  setGaussianClustersData() {
    // Get Gaussian clusters data
    this.trainingData = DataGenerator.generateGaussianClustersData(200);
    ...
    // Update configuration
    this.config.inputSize = 2;
    this.config.outputSize = 3; // 3 clusters = 3 outputs
    this.updateConfigUI();
    this.createNetwork();
  }
        ...
        const history = await this.mlp.train(
          ...
          (epoch, loss, metric, isMAE) => {
            ...
            if (epoch % 10 === 0 || epoch === this.config.epochs - 1) {
              ...
              document.getElementById('epoch-display').textContent = `Epoch: ${epoch + 1}/${
                this.config.epochs
              }`;
  ...
  updateStatsUI() {
    ...
    // Update loss value
    const lastLoss =
      this.mlp.history.loss?.length > 0
    ...
    if (isSineWave) {
      ...
      // For classification, display Accuracy percentage
      const lastAccuracy =
        this.mlp.history.accuracy?.length > 0
    ...
    document.getElementById('dataset-type').addEventListener('change', e => {
      ...
      switch (datasetType) {
        ...
        case 'moons':
          this.setMoonsData();
          break;
        case 'gaussian-clusters':
          this.setGaussianClustersData();
          break;
      }
